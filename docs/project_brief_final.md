...
![Azure ML Studio - MLflow Run History](images/azure_mlflow_runs.png)
...
![Azure Functions - Daily Pipeline Execution Logs](images/azure_functions_logs.png)
...
![Azure Storage Explorer - Blob Data Lake](images/azure_storage_containers.png)
...
![GitHub Actions - CI/CD Workflow for SWA](images/github_actions_workflow.png)
...
*   **`notebooks/eda.ipynb`**: Kode lengkap Exploratory Data Analysis, visualisasi tren harga historis, dan analisis korelasi antar-komoditas pangan. (Path Relatif: `../notebooks/eda.ipynb`)
*   **`notebooks/evaluate_prophet.ipynb`**: Kode eksperimen pemodelan Prophet, tuning parameter, dan pengujian model time-series. (Path Relatif: `../notebooks/evaluate_prophet.ipynb`)
*   **`notebooks/analysis_walkthrough.ipynb`**: Panduan end-to-end (walkthrough) alur data dari ETL, pemodelan, hingga output serving dashboard. (Path Relatif: `../notebooks/analysis_walkthrough.ipynb`)

**2. Dokumen Analisis & Panduan Teknis (Format `.md`):**
*   **`docs/data_dictionary.md`**: Kamus data terstruktur yang mendefinisikan skema data mentah (JSON) PIHPS, data serving dashboard, dan skema logging metrik. (Path Relatif: `data_dictionary.md`)
*   **`docs/eda_interpretation.md`**: Interpretasi bisnis hasil EDA, analisis lonjakan harga musiman (Meugang & Ramadan), serta temuan anomali harga pangan di Aceh. (Path Relatif: `eda_interpretation.md`)
*   **`docs/evaluate_prophet.md`**: Laporan evaluasi performa model Prophet harian, perbandingan metrik kesalahan (MAPE, MAE, RMSE), dan analisis error model. (Path Relatif: `evaluate_prophet.md`)
*   **`docs/05_arm_workflow_dataflow.md`**: Laporan visualisasi diagram alur data (*data flow*) harian, arsitektur MLOps, serta alur pemicuan otomatis bot notifikasi Telegram. (Path Relatif: `05_arm_workflow_dataflow.md`)
